  ...
  methods: {
    ...
    fetchAttrList(attrGroupId) {
      attrApi.getList(attrGroupId).then(response => {
        ...
        // 修改属性回显
        // debugger
        if (this.skuAttrValueList.length > 0) {
          this.attrList.forEach(attrItem => {
            this.skuAttrValueList.forEach(valueItem => {
    ...
    saveData() {
      ...
      this.skuAttrValueList = attrListArray
      if (this.skuImagesList.length > 0) {
        this.skuInfo.imgUrl = this.skuImagesList[0].imgUrl
      }
    ...
    fetchDataById(id) {
      api.getById(id).then(response => {
        ...
        // 图片回显
        console.log(this.skuImagesList)
        this.skuImagesList.forEach(item => {
          let obj = new Object()
          obj.url = item.imgUrl
          this.fileList.push(obj)
        })

        this.skuPosterList.forEach(item => {
          let obj = new Object()
          obj.url = item.imgUrl
